...
import {
  CARD_COLORS as VIDERE_CARD_COLORS,
  CARD_TYPES as VIDERE_CARD_TYPES,
  type CARD_RARITIES as GENERATED_CARD_RARITIES,
  type CardType,
} from '@videreproject/constants'

import { CARD_FORMATS, type CardSearchFormat } from './formats'

export { CARD_FORMATS }
export type { CardSearchFormat }

export const COLORLESS_CARD_COLOR = 'C' as const

/** WUBRG from `@videreproject/constants`, plus colorless for search UI. */
export const CARD_COLORS = [
  ...VIDERE_CARD_COLORS.map(color => color.symbol),
  COLORLESS_CARD_COLOR,
] as const

/** Stable OpenAPI legality values used by card search. */
...
/** Display order for rarity filter chips (presentation preference, not DB rank). */
export const CARD_RARITIES = [
  ...
  'promo',
  'token',
] as const satisfies readonly (typeof GENERATED_CARD_RARITIES)[number][]
...
export type CardTypeFilter = Lowercase<CardType>
export const CARD_TYPE_FILTERS = VIDERE_CARD_TYPES.map(type => type.toLowerCase()) as readonly CardTypeFilter[]
...
export function buildCardSearchQuery(baseQuery: string, filters: CardFilterState) {
  ...
  if (filters.rarity !== 'any') {
    ...
    terms.push(
      filters.rarityOperator === 'any' || filters.rarityOperator === '='
        ? `r:${rarity}`
