...
import { getMtgoChatSymbolUrl } from './videre-cdn'
...
const NORMALIZED_CHAT_SYMBOL_PATTERN = /^[a-z0-9]+(?:[\s_-]+[a-z0-9]+)*$/
...
export function getMtgoChatMarkupSymbol(symbol: string): MtgoChatMarkupSymbol | null {
  const token = symbol.trim()
  if (!NORMALIZED_CHAT_SYMBOL_PATTERN.test(token)) return null
  return { type: 'chatSymbol', value: token }
}
