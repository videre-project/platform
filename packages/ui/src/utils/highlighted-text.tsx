...
export function HighlightedText({
  ...
  while (matchIndex >= 0) {
    if (matchIndex > cursor) {
      nodes.push(<React.Fragment key={`t-${index++}`}>{text.slice(cursor, matchIndex)}</React.Fragment>)
    }
  ...
  if (cursor < text.length) {
    nodes.push(<React.Fragment key={`t-${index}`}>{text.slice(cursor)}</React.Fragment>)
  }
