...
import { FORMAT_CODES, type FormatCode } from '@videreproject/constants'
...
const FORMAT_DEFINITIONS = FORMAT_CODES.map(id => ({
  id,
  ...FORMAT_PRESENTATION[id],
}))

/** Wire-format card-search format codes (lowercase), sourced from `@videreproject/constants`. */
export type CardSearchFormat = FormatCode

export const CARD_FORMATS = [...FORMAT_CODES]

const FORMAT_MATCH_ORDER = [...FORMAT_DEFINITIONS].sort((a, b) => b.id.length - a.id.length)

function findFormat(format: string) {
  const normalizedFormat = format.trim().toLowerCase()
  return (
    FORMAT_DEFINITIONS.find(candidate => candidate.id === normalizedFormat) ??
