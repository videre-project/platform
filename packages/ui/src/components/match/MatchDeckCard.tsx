...
import type { MatchDetailsCard } from '../../types/match'

export function DeckManaSymbols({ colors }: { colors?: string[] | null }) {
  if (!colors) return null
  const visibleColors = colors.length ? colors : ['C']
    ...
    <>
      <div className="relative z-0 h-36 overflow-visible rounded-t-lg border-b border-sidebar-border/60">
        ...
        <div className="absolute inset-x-0 bottom-0 top-0 z-10 [clip-path:inset(-4rem_-5rem_1px_-5rem)]">
          {previewCards.length ? previewCards.slice(0, 5).map((card, index, cards) => {
            ...
            <div className="absolute inset-x-6 bottom-3 grid grid-cols-5 gap-2">
              {Array.from({ length: 5 }).map((_, index) => (
                <div key={index} className="aspect-[5/7] rounded-sm border border-sidebar-border/60 bg-muted/45" />
              ))}
            </div>
          )}
        </div>
      ...
      {interactive ? (
        <span className="absolute right-3 top-3 z-30 inline-flex h-8 w-8 items-center justify-center rounded-md border border-sidebar-border/70 bg-background/85 text-foreground opacity-0 shadow-sm backdrop-blur-sm transition-opacity group-hover/editor:opacity-100 group-focus-visible/editor:opacity-100" aria-hidden="true">
      ...
      <div className="relative z-20 rounded-b-lg bg-card p-3">
        <div className="flex min-w-0 items-start justify-between gap-3">
          <div className="min-w-0 pt-1">
            ...
            <p className="mt-0.5 truncate text-xs leading-4 text-muted-foreground">{deckArchetype || 'Unclassified deck'}</p>
          </div>
        </div>
      </div>
    </>
...
export function MatchDeckCard({
  ...
  const className = cn(
    ...
    onOpen && 'hover:z-20 hover:border-primary/35 hover:bg-muted/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring',
  )

  if (!onOpen) {
    return (
      <div className={className}>
        <MatchDeckCardContent deckName={deckName} deckArchetype={deckArchetype} deckColors={deckColors} backgroundArtUrl={backgroundArtUrl} previewCards={previewCards} interactive={false} />
      </div>
    )
  }
  ...
  return (
    <button type="button" onClick={onOpen} className={className}>
      <MatchDeckCardContent deckName={deckName} deckArchetype={deckArchetype} deckColors={deckColors} backgroundArtUrl={backgroundArtUrl} previewCards={previewCards} interactive />
    </button>
  )
}
