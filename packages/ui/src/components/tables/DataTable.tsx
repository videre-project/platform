...
export function DataTable<TData, TValue>({
  ...
  const lastNavigatedSelectionRef = useRef<{ id: string | null; key: number | undefined } | null>(
    null,
  )
                    ...
                    {cellIndex === 0 && row.getCanExpand() ? (
                      <span className="inline-flex items-center gap-1">
                        <button
                          ...
                          className="cursor-pointer p-0.5 text-muted-foreground transition-transform hover:text-foreground"
  ...
  const bodyScrollStyle: CSSProperties = {
    ...
    ...(bodyScrollSize.scrollbarWidth > 0 ? { marginRight: -bodyScrollSize.scrollbarWidth } : {}),
  }
  const bodyTableStyle = bodyScrollSize.width > 0 ? { width: bodyScrollSize.width } : undefined
      ...
      <div className={cn('rounded-md', tableContainerClassName)}>
        {bodyWrapperClassName ? (
          <>
            ...
            <div ref={bodyScrollRef} className={bodyWrapperClassName} style={bodyScrollStyle}>
              <Table
