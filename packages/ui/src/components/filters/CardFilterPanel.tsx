...
export type CardFilterPanelProps = {
...
export function CardFilterPanel({
  ...
  const cycleType = (type: CardTypeFilter) => {
    ...
    const next: CardTypeFilterState =
      current === 'off' ? 'include' : current === 'include' ? 'exclude' : 'off'
    onUpdate({ typeStates: { ...filters.typeStates, [type]: next } })
  }

  const updateBoolean = (key: 'promoMode' | 'multifaceMode' | 'splitMode', value: CardBooleanMode) =>
    onUpdate({ [key]: value })
  ...
  return (
    <div className={cn('flex min-h-0 flex-1 flex-col overflow-hidden', className)}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
        <div className="grid grid-cols-1 gap-y-3">
          <section className={sectionClass}>
            ...
            <Select
              value={filters.textMode}
              onValueChange={value => onUpdate({ textMode: value as CardSearchTextMode })}
            >
              <SelectTrigger className={cn(controlClass, 'w-full')}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {CARD_SEARCH_TEXT_MODES.map(mode => (
          ...
          <section className={sectionClass}>
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                onValueChange={value => onUpdate({ colorMode: value as CardColorMode })}
              >
                <SelectTrigger className={cn(controlClass, 'w-32 shrink-0')}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {CARD_COLOR_MODES.map(mode => (
                    <SelectItem key={mode.value} value={mode.value}>
                      {mode.label}
                    </SelectItem>
                  ))}
            ...
            <div className="grid gap-2">
              <div className="grid grid-cols-[7rem_1fr] gap-2">
                <Select
                  ...
                  onValueChange={value =>
                    onUpdate({ manaValueOperator: value as CardComparisonOperator })
                  }
                >
                ...
                <Input
                  ...
                  onChange={event =>
                    onUpdate({ manaValue: event.target.value.replace(/[^\d]/g, '') })
                  }
                  inputMode="numeric"
              ...
              <Input
                ...
                onChange={event => onUpdate({ manaCost: event.target.value })}
          ...
          <section className={sectionClass}>
            ...
            <div className="grid grid-cols-[7rem_1fr] gap-2">
              <Select
                ...
                onValueChange={value =>
                  onUpdate({ rarityOperator: value as CardComparisonOperator })
                }
              >
                <SelectTrigger className={controlClass}>
                  <SelectValue />
              ...
              <Select
                ...
                onValueChange={value => onUpdate({ rarity: value as CardRarityFilter })}
              >
                <SelectTrigger className={controlClass}>
                  <SelectValue />
                </SelectTrigger>
          ...
          <section className={sectionClass}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Select
                ...
                onValueChange={value => onUpdate({ format: value as CardFormatFilter })}
              >
                <SelectTrigger className={controlClass}>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Select
                ...
                onValueChange={value => onUpdate({ legality: value as CardLegalityFilter })}
                disabled={filters.format === 'any'}
              >
                <SelectTrigger className={controlClass}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {CARD_LEGALITIES.map(legality => (
                    <SelectItem key={legality} value={legality}>
                      {titleCaseCardFilter(legality)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <section className={sectionClass}>
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              <Input
                ...
                onChange={event => onUpdate({ setCode: event.target.value })}
                placeholder="Set code"
                className={controlClass}
              />
              <Input
                value={filters.collectorNumber}
                onChange={event => onUpdate({ collectorNumber: event.target.value })}
                placeholder="Collector no."
                className={controlClass}
              />
              <Input
                value={filters.catalogId}
                onChange={event =>
                  onUpdate({ catalogId: event.target.value.replace(/[^\d]/g, '') })
                }
                inputMode="numeric"
                placeholder="MTGO id"
                className={controlClass}
              ...
              <Input
                value={filters.artId}
                onChange={event => onUpdate({ artId: event.target.value.replace(/[^\d]/g, '') })}
                inputMode="numeric"
                placeholder="Art id"
                className={controlClass}
              />
              <Input
                ...
                onChange={event => onUpdate({ frameStyle: event.target.value })}
                placeholder="Frame"
                className={controlClass}
              />
              <Input
                value={filters.promoLabel}
                onChange={event => onUpdate({ promoLabel: event.target.value })}
              ...
              <Input
                ...
                onChange={event => onUpdate({ flavor: event.target.value })}
                placeholder="Flavor text"
                className={cn(controlClass, 'sm:col-span-2')}
              />
            </div>
          </section>
          ...
          <section className={sectionClass}>
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Select
                value={filters.tokenMode}
                onValueChange={value => onUpdate({ tokenMode: value as CardTokenFilterMode })}
              >
                <SelectTrigger className={controlClass}>
                  <SelectValue />
                </SelectTrigger>
              ...
              {(
                ...
                <Select
                  key={key}
                  value={filters[key]}
                  onValueChange={value => updateBoolean(key, value as CardBooleanMode)}
                >
                  <SelectTrigger className={controlClass}>
      ...
      <div className="flex items-center justify-between gap-2 border-t border-sidebar-border/60 p-2">
        <Button variant="ghost" size="sm" onClick={onClear} disabled={activeFilterCount === 0}>
          Clear filters
        </Button>
        <Button
          variant="outline"
          size="sm"
          onClick={onClose}
          className="border-sidebar-border/70 bg-background/70"
        >
          Close
        </Button>
      </div>
