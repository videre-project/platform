  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center gap-2 overflow-hidden pr-8">
        <div className="min-w-0 truncate text-sm font-medium text-foreground">{card.name}</div>
        {card.setCode ? (
          <span className="shrink-0 rounded-md border border-sidebar-border/70 bg-background/70 px-1.5 py-0.5 text-center text-[11px] font-medium uppercase text-muted-foreground">
            {card.setCode}
          </span>
        ) : null}
      </div>
      ...
      {selected ? (
        <div className="mt-1.5 min-w-0 whitespace-pre-wrap text-xs leading-5 text-muted-foreground">
          ...
          <GameLogText
            ...
            manaSymbolClassName="mx-0.5 inline h-3.5 w-3.5 align-text-bottom"
          />
          {powerText ? (
            <>
              <span className="float-right ml-1 mt-0.5 inline-flex items-center rounded-sm bg-background/70 px-1 py-0 text-[11px] font-medium leading-4 text-foreground/85 ring-1 ring-sidebar-border/65">
                {powerText}
              </span>
...
function MiniDiffCardItem({ catalogId, borderClass }: { catalogId: number; borderClass: string }) {
  ...
  return (
    <div
      {...tooltipHandlers}
      className={cn(
        'relative h-[26px] w-full cursor-pointer overflow-hidden border-t bg-black/40 first:border-t-0',
        borderClass,
      )}
    >
      <CardImage
        catalogId={catalogId}
        alt=""
        className="pointer-events-none h-auto w-full select-none object-top"
      />
    </div>
  )
}
...
function MiniCardDiffColumns({ changes }: { changes: DeckHistoryChange[] }) {
  const additions = useMemo(() => {
    ...
    for (const c of changes) {
      if (c.quantityDelta > 0) {
        for (let i = 0; i < c.quantityDelta; i++) list.push(c.catalogId)
      }
    }
    return list
  }, [changes])
  ...
  const removals = useMemo(() => {
    ...
    for (const c of changes) {
      if (c.quantityDelta < 0) {
        for (let i = 0; i < Math.abs(c.quantityDelta); i++) list.push(c.catalogId)
      }
    }
    return list
  }, [changes])
  ...
  return (
    <div className="-mx-2.5 -mb-2.5 mt-1.5 overflow-hidden rounded-b-lg border-t border-sidebar-border/50 bg-background/40 px-2.5 pb-0 pt-1.5">
      <div className="grid w-full grid-cols-2 items-stretch gap-3 divide-x divide-sidebar-border/30 pb-0">
        <div className="flex h-full min-w-0 flex-col gap-1.5 pb-0 pr-0.5">
          ...
          {additions.length > 0 ? (
            <div className="flex flex-col -space-y-px overflow-hidden rounded-t border border-b-0 border-emerald-500/40 shadow-sm ring-1 ring-emerald-500/20">
            ...
            <div className="flex min-h-[26px] w-full flex-1 items-center justify-center rounded-t border border-b-0 border-dashed border-sidebar-border/40 bg-background/20 text-[10px] font-medium text-muted-foreground/30">
              None
            </div>
          )}
        </div>

        <div className="flex h-full min-w-0 flex-col gap-1.5 pb-0 pl-3">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-rose-400/90">
            <span className="truncate">Removed ({removals.length})</span>
          </div>
          {removals.length > 0 ? (
            <div className="flex flex-col -space-y-px overflow-hidden rounded-t border border-b-0 border-rose-500/40 shadow-sm ring-1 ring-rose-500/20">
...
/**
 * Deck editor tools side pane (card search, history, placeholders).
 * Host owns card search fetch and history selection; this pane owns filter UI state.
 */
export function DeckBuildSidePane({
  ...
  const [internalFilters, setInternalFilters] = useState<CardFilterState>(DEFAULT_CARD_FILTERS)
  const cardFilters = controlledFilters ?? internalFilters
  ...
  useEffect(() => {
    onSearchQueryChangeRef.current?.(effectiveSearchQuery)
  }, [effectiveSearchQuery])

  useEffect(() => {
    if (view !== 'cards') setFiltersOpen(false)
  }, [view])

  useEffect(() => {
    if (isCollapsed) setFiltersOpen(false)
  }, [isCollapsed])
  ...
  }, [updateResultsFade, searchResults.length])

  const updateCardFilters = useCallback(
    (updates: Partial<CardFilterState>) => {
      const next = { ...cardFilters, ...updates }
      if (controlledFilters === undefined) setInternalFilters(next)
      onCardFiltersChange?.(next)
    },
    [cardFilters, controlledFilters, onCardFiltersChange],
  )

  const clearCardFilters = useCallback(() => {
    if (controlledFilters === undefined) setInternalFilters(DEFAULT_CARD_FILTERS)
    onCardFiltersChange?.(DEFAULT_CARD_FILTERS)
  }, [controlledFilters, onCardFiltersChange])

  const tabClass = (tab: DeckSidePanelView) =>
    cn(
                ...
                {historyLoading ? (
                  <div className="flex flex-col gap-2">
                    {[0, 1, 2].map(i => (
                      <div
                        key={i}
                        className="h-20 space-y-2 rounded-md border border-sidebar-border/50 bg-background/35 p-3"
                      >
                  ...
                  historyData.revisions.map((rev, index) => {
                    ...
                    const isSelected =
                      ...
                      (selectedRevisionId === null && isLatest)
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          'group relative cursor-pointer overflow-hidden rounded-lg border px-2.5 py-1.5 text-left transition-all',
                          isSelected
                            ? 'border-primary bg-secondary/60 shadow-sm ring-1 ring-primary/40'
                            : 'border-sidebar-border/60 bg-background/50 hover:border-sidebar-border hover:bg-background/80',
                        )}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex min-w-0 items-center gap-1.5">
                            <span
                              className={cn(
                                '-ml-1 shrink-0 rounded px-1 py-0.5 text-[10px] font-semibold uppercase leading-none tracking-wider',
                                isLatest
                            ...
                            >
                              {isLatest
                                ? 'Latest'
                                : `v${historyData.revisions.length - index}`}
                            </span>
                            {isInitial ? (
                          ...
                          <span className="shrink-0 truncate text-xs font-medium leading-none text-muted-foreground">
              ...
              <div className="shrink-0">
                <div className="relative">
                  ...
                  <button
                    ...
                    aria-label={
                      filtersOpen ? 'Close card query builder' : 'Open card query builder'
                    }
              ...
              <div className="flex min-h-0 flex-1 flex-col">
                <div className="relative min-h-0 flex-1">
                  <div
                    ...
                    className="h-full min-h-0 overflow-x-hidden overflow-y-auto pb-3"
                    style={{
                      width:
                        resultsScrollbarWidth > 0
...
/** Alternate name for the deck tools pane. */
export const DeckToolsPane = DeckBuildSidePane
