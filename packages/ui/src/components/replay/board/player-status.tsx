...
import { useCardMedia } from "../../cards/CardMediaProvider"
...
export function PlayerAvatar({
  ...
  const { getCardArtUrl } = useCardMedia()
  const [artFailed, setArtFailed] = useState(false)
  const [fallbackFailed, setFallbackFailed] = useState(false)
  const artUrl = player.avatarId > 0 ? getCardArtUrl?.(player.avatarId) ?? null : null

  useEffect(() => {
    setArtFailed(false)
    setFallbackFailed(false)
  }, [artUrl, defaultAvatarUrl, player.avatarId])

  const showClock = clockSeconds != null && clockSeconds > 0
  const counters = Object.entries(player.counters)
    .filter(([, count]) => count > 0)

  // Do not race the API art request against an invented /avatars URL. A host
  // fallback is optional and never suppresses art that resolves later.
  const displayAvatarUrl = artUrl && !artFailed
    ? artUrl
    : !fallbackFailed
      ...
      <div className="relative w-8 h-8 rounded overflow-hidden shrink-0 bg-muted/40">
        {displayAvatarUrl ? (
          <img
            ...
            onError={() => {
              if (artUrl && !artFailed) setArtFailed(true)
