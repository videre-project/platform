...
import { backfaceUrl as defaultBackfaceUrl } from "../../../assets"
                ...
                <TooltipTrigger asChild>
                  <div className="rounded-md overflow-hidden border border-sidebar-border/60 bg-muted/20" style={{ aspectRatio: "5/7" }}>
                    <CardImg
                      ...
                      name={card.name}
                      resolveFace={Boolean(card.initialName && card.name !== card.initialName)}
                      alt={card.name}
...
export function BottomZoneBar({
  ...
  promptText,
  promptOptions,
  cardBackUrl = defaultBackfaceUrl,
}: {
  hand: CardState[]
  zones: ZonePileEntry[]
            ...
            <div key={zone.label} className="flex flex-col items-start">
              ...
              >
                {zone.isLibrary && zone.count > 0 ? (
                  <img src={cardBackUrl} alt="Library" className="w-full h-full object-cover" />
                ) : zone.topCard ? (
                  <motion.div layoutId={`card-${zone.topCard.lineageId}`} layout transition={CARD_TRANSITION} className="absolute inset-0">
                    <CardImg
                      ...
                      resolveFace={Boolean(zone.topCard.initialName && zone.topCard.name !== zone.topCard.initialName)}
...
export function TopZoneBar({
  ...
  return (
    <div
      className={[
        isExpanded || browseZone ? "absolute inset-x-0 top-0 z-30" : "absolute inset-x-0 top-0 z-0",
        disableHoverExpansion ? "cursor-not-allowed" : "",
      ].filter(Boolean).join(" ")}
      data-ui-replay-top-zones
      data-expanded={isExpanded ? "true" : "false"}
      onMouseEnter={disableHoverExpansion ? undefined : () => setIsExpanded(true)}
      onMouseLeave={disableHoverExpansion ? undefined : () => setIsExpanded(false)}
    >
      <div className={["overflow-hidden", "transition-[height,box-shadow] duration-300 ease-in-out", isExpanded ? "border-b border-sidebar-border/60 shadow-xl" : ""].join(" ")} style={{ height: isExpanded ? TOP_BAR_FULL : TOP_BAR_PEEK }}>
        <div className="flex bg-background" style={{ height: TOP_BAR_FULL }}>
                ...
                <AnimatePresence>
                  ...
                  {Array.from({ length: hiddenHandCount }, (_, i) => {
                    ...
                    return (
                      ...
                      >
                        <img src={cardBackUrl} alt="Hidden card" className="w-full h-full object-cover" />
                      </motion.div>
                    )
                  })}
                </AnimatePresence>
              ...
              return (
                <div key={zone.label} className="flex flex-col items-start">
                  ...
                  <div
                    ...
                    className={[
                      "rounded-md border border-sidebar-border/60 overflow-hidden relative bg-muted/20",
                      browsable ? (disableHoverExpansion ? "!cursor-not-allowed group/zone" : "cursor-pointer group/zone") : "",
                      browsable ? "transition-[border-color,box-shadow] hover:border-sidebar-accent/70 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.15)]" : "",
                      isBrowseOpen ? "border-sidebar-accent ring-1 ring-sidebar-accent/70" : "",
                    ].filter(Boolean).join(" ")}
                    style={{ width: ZONE_CARD_W, height: ZONE_CARD_H }}
...
function optionLabel(action: GameAction): string {
  switch (action.$type) {
    ...
    case "DistributingCardAction": {
      const card = parseCardName(action.card)
      return card
        ? `${action.name ?? "Play"}: ${card}`
        : action.name ?? "Play"
    }
    case "SelectFromListAction": {
      const items = action.availableItems
    ...
    case "NumericAction": {
      return `Choose (${action.minimum ?? 0}–${action.maximum ?? "?"})`
    }
